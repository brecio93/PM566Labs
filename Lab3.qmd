...
-   How many columns, rows are there? 2377343 rows and 30 columns

```{r}
dim(met) #this is number of rows and columns "dimension of the dataset"
head(met)
tail(met)
```
...
```{r}
str(met) #structure of dataset
```

### **4. Take a closer look at the key variables.**

```         
this is useful for understanding the data before doing any analysis or visualization. for example, for continous data you could use histagrams and for categorical data barplots
```
...
```         
Goal: undertand what the variables and data points mean 
```
...
# Now your map code will work
leaflet(elev) %>%
  addProviderTiles("OpenStreetMap") %>% 
  addCircles(
    lat = ~lat,
    lng = ~lon,   # or ~long, depending on your column name
    opacity = 1,
    fillOpacity = 1,
    radius = 100
  )
...
# Build a proper datetime string "YYYY-MM-DD HH"
elev$date <- with(
  elev,
